...
export abstract class MultiLingualEntity {
  ...
  async save(): Promise<any> {
    ...
    const url = create ? this.createUrl : `${this.createUrl}/${this.document.id}`;
    const response = await globals.api.post(url, data, requestConfig);
  ...
  getValue(attribute: string) {
    const { langcode } = window.app.language;
