...
export default class Organization extends MultiLingualEntity implements EntityInterface, MultiLingualEntityInterface {
  ...
  async load(): Promise<any> {
    ...
    if(globals.app.user.id) {
      let response = await window.api.get(`/a/app/organization/${this.document.id}/me`)
  ...
  protected setIncluded(included: any) {
    ...
    let files = included?.filter(row => row.type == 'file--file')
    if(files?.length) {
  ...
  public getTitle(): string {
    let lang = globals.app.language.langcode
