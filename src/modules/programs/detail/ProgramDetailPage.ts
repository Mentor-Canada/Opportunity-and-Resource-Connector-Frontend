...
export default {
  ...
  async mounted() {
    ...
    Vue.nextTick(() => {
      let organizationDetailPage = new Vue(OrganizationDetailPage)
      organizationDetailPage.programMode = true
      let organizationMountEl = this.$el.querySelector('#organization-mount')
  ...
  methods: {
    getRequiredTitle() {
      let en = this.program.localizedAttributes['en'].field_display_title
      if(globals.app.languages.list.length == 1) {
        return en
      }
      let secondaryLangcode = this.app.languages.getSecondaryLangcode()
      let fr = this.program.localizedAttributes[secondaryLangcode].field_display_title
      if(en == '' && fr != '') {
        return fr
      }
      return en
    ...
    async submit() {
      let fileEl: HTMLInputElement = document.querySelector('#program-logo-file')
      ...
      if(this.app.view.isAdminPage) {
        ...
        let title = this.program.localizedAttributes["en"].field_display_title
        let email = this.program.attributes.contactEmail
        let organizationId = this.program.attributes.organization
