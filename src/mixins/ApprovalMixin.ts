...
export default {
  ...
  data() {
    return {
      ...
      sortOrder: [{
        name: 'attributes.title', direction: 'asc', field: 'attributes.title', sortField: 'attributes.title',
      }],
      fields: [
  ...
  methods: {
    ...
    getSortParam(sortOrder): string {
      return this.sortOrder.map((sort) => {
    ...
    transformData(data) {
      ...
      for (const row of data.data) {
        ...
        const includedId = row.relationships.field_user_entity.data?.id;
