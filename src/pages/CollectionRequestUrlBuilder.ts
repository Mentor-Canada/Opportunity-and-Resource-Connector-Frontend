...
export default class CollectionRequestUrlBuilder {
  ...
  sort(values: any[]) {
    this.params.sort = values.map((sort) => {
  ...
  start(value: string, timestamp = true) {
    ...
    if (value && value != '') {
      const time = this.getTime(value) + (24 * 60 * 60);
  ...
  build(): string {
    ...
    const urlString = components.join('/');
    return urlString;
  }
  ...
  private getTime(value: any): number {
    if (value.split) {
      ...
      const utcDate = new Date(Date.UTC(Number(components[0]), Number(components[1]) - 1, Number(components[2])));
